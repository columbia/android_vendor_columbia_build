...
__setup_device_for_debug() {
	# Check bind-mounts
	if [ $COLUMBIA_REXEC_EN -eq 1 ]; then
		__perform_remote_cmd "$_ADB shell mount |grep '/usr'"
		usrmnt=$(cat $COLUMBIA_REXEC_OUTPUT)
		__perform_remote_cmd "$_ADB shell mount |grep '/System'"
		sysmnt=$(cat $COLUMBIA_REXEC_OUTPUT)
		__perform_remote_cmd "$_ADB shell mount |grep '/Library'"
		libmnt=$(cat $COLUMBIA_REXEC_OUTPUT)
		__perform_remote_cmd "$_ADB shell mount |grep '/Applications'"
		appmnt=$(cat $COLUMBIA_REXEC_OUTPUT)
		__perform_remote_cmd "$_ADB shell mount |grep '/shm'"
		shmmnt=$(cat $COLUMBIA_REXEC_OUTPUT)
		__perform_remote_cmd "$_ADB shell mount |grep '/private'"
		privmnt=$(cat $COLUMBIA_REXEC_OUTPUT)
		__perform_remote_cmd "$_ADB shell mount |grep '/sbin'"
		sbinmnt=$(cat $COLUMBIA_REXEC_OUTPUT)
		__perform_remote_cmd "$_ADB shell mount |grep '/var'"
		varmnt=$(cat $COLUMBIA_REXEC_OUTPUT)
	else
		usrmnt=`$_ADB shell mount |grep '/usr'`
		sysmnt=`$_ADB shell mount |grep '/System'`
		libmnt=`$_ADB shell mount |grep '/Library'`
		appmnt=`$_ADB shell mount |grep '/Applications'`
		shmmnt=`$_ADB shell mount |grep '/shm'`
		privmnt=`$_ADB shell mount |grep '/private'`
		sbinmnt=`$_ADB shell mount |grep '/sbin'`
		varmnt=`$_ADB shell mount |grep '/var'`
	fi

	if [ -z "$usrmnt" ]; then
		__device_create_dir "/usr"
		__device_bind_mnt "/data/ios/usr" "/usr"
	fi

	if [ -z "$sysmnt" ]; then
		__device_create_dir "/System"
		__device_bind_mnt "/data/ios/System" "/System"
	fi

	if [ -z "$libmnt" ]; then
		__device_create_dir "/Library"
		__device_bind_mnt "/data/ios/Library" "/Library"
	fi

	if [ -z "$appmnt" ]; then
		__device_create_dir "/Applications"
		__device_bind_mnt "/data/ios/Applications" "/Applications"
	fi

	if [ -z "$shmmnt" ]; then
		__device_create_dir "/shm"
		__device_bind_mnt "/data/ios/shm" "/shm"
	fi

	if [ -z "$privmnt" ]; then
		__device_create_dir "/private"
		__device_bind_mnt "/data/ios/private" "/private"
	fi

	if [ -z "$sbinmnt" ]; then
		__device_create_dir "/sbin"
		__device_bind_mnt "/data/ios/sbin" "/sbin"
		__device_symlink "/sbin" "/bin"
	fi

	if [ -z "$varmnt" ]; then
		__device_create_dir "/var"
		__device_bind_mnt "/data/ios/private/var" "/var"
	fi
	# top-level symlinks
	__device_create_dir "/private/var/tmp"
	__device_symlink "/private/var/tmp" "/tmp"
	__device_symlink "/var/mobile" "/User"
	__device_symlink "/data/ios/etc/master.passwd" "/etc/master.passwd"
	__device_symlink "/data/ios/etc/master.passwd" "/etc/passwd"
	__device_symlink "/data/ios/etc/asl.conf" "/etc/asl.conf"
	__device_symlink "/data/ios/etc/launchd.conf" "/etc/launchd.conf"
	__device_symlink "/data/ios/etc/notify.conf" "/etc/notify.conf"
	__device_symlink "/data/ios/etc/group" "/etc/group"
	__device_create_dir "/var/root"
	__device_create_dir "/var/db/launchd.db/com.apple.launchd"
	__device_create_dir "/var/run"

	# Disable address space randomization
	if [ $COLUMBIA_REXEC_EN -eq 1 ]; then
		__perform_remote_cmd "$_ADB shell 'if [ ! -f /dev/xnulog ]; then echo /dev/xnulog > /d/xnu/log; fi';"
		__perform_remote_cmd "$_ADB shell 'echo 0 > /proc/sys/kernel/randomize_va_space'; $_ADB forward tcp:1111 tcp:1111"
	else
		$_ADB shell 'if [ ! -f /dev/xnulog ]; then echo /dev/xnulog > /d/xnu/log; fi'
		$_ADB shell 'echo 0 > /proc/sys/kernel/randomize_va_space'
		$_ADB forward tcp:1111 tcp:1111
	fi
	# go ahead and make this the default
	__setup_launchd_for_debug
}
...
__setup_launchd_for_debug() {
	if [ $COLUMBIA_REXEC_EN -eq 1 ]; then
		...
		REXEC=$(cat <<-SETVAR
			START_COMMAND
			#!/bin/bash
			$_ADB shell touch /var/db/.debug_launchd
			$_ADB shell touch /var/db/.launchd_log_per_user_shutdown
			$_ADB shell touch /var/db/.launchd_log_pid1_shutdown
			$_ADB shell touch /var/db/.launchd_shutdown_debugging
			$_ADB shell "if [ -c /dev/console ]; then mv /dev/console /dev/real_console; fi"
			$_ADB shell "if [ ! -f /dev/console ]; then touch /dev/console; fi"
			$_ADB shell chmod 755 /System/LibraryLaunchDaemons
			$_ADB shell "find /System/Library/LaunchDaemons -type f -exec chmod 644 {} \;"
			___END_COMMAND___
		SETVAR)
		echo "$REXEC" > $COLUMBIA_REXEC_FILE
		$RDC "$COLUMBIA_REXEC_FILE" $COLUMBIA_REXEC_OUTPUT 'echo' <&0
		rm -f "$COLUMBIA_REXEC_FILE"
	else
		$_ADB shell "touch /var/db/.debug_launchd; \
			     touch /var/db/.launchd_log_per_user_shutdown; \
			     touch /var/db/.launchd_log_pid1_shutdown; \
			     touch /var/db/.launchd_shutdown_debugging; \
			     if [ -c /dev/console ]; then mv /dev/console /dev/real_console; fi; \
			     if [ ! -f /dev/console ]; then touch /dev/console; fi; \
			     chmod 755 /System/Library/LaunchDaemons; \
			     find /System/Library/LaunchDaemons -type f -exec chmod 644 {} \;"
	fi
}

#
# Libraries we insert using "DYLIB_INSERT_LIBRARIES" iOS environment variable
#
__IOS_INSERT_LIBS=/usr/lib/wrappers/pthread_wq_np.dylib:/usr/lib/wrappers/libkqueue.dylib:/usr/lib/wrappers/libswizzle.dylib
__IOS_DYLD_VARS="DYLD_FORCE_FLAT_NAMESPACE=1 DYLD_INSERT_LIBRARIES=${__IOS_INSERT_LIBS}"
...
function ios_dbg() {
	...
	if [ $COLUMBIA_REXEC_EN -eq 1 ]; then
		...
		REXEC=$(cat <<-SETVAR
			START_COMMAND
			#!/bin/bash
			$_ADB shell "${__IOS_DYLD_VARS} gdbserver 127.0.0.1:1111 $@"&
			sleep 1
			$_ARM_GDB -nx \
				-ex "source $_IOS_GDBINIT" \
...
function ios_run() {
	...
	# do a little manual 'path' magic to shorten command line...
	local binpath=
	local _firstchar=${bin:0:1}
	if [ "$_firstchar" = "/" ]; then
		binpath=$bin
	else
		local __dirs="/data/ios/Applications /data/ios/sbin /usr/bin /system/bin /sytem/xbin"
		if [ $COLUMBIA_REXEC_EN -eq 1 ]; then
			__perform_remote_cmd "$_ADB shell \"find $__dirs -type f -name $bin 2>/dev/null\""
			binpath=$(cat $COLUMBIA_REXEC_OUTPUT)
		else
			binpath=$($_ADB shell "find $__dirs -type f -name $bin 2>/dev/null")
		fi
		# ADB appends a stupidly annoying carriage return... ARRRRGGGHHH!
		if [ ${#binpath} -gt 0 ]; then
			((len=${#binpath}-1))
			binpath=${binpath:0:$len}
		fi
	fi
	if [ -z "$binpath" ]; then
		binpath=$bin
	...
	if [ $COLUMBIA_REXEC_EN -eq 1 ]; then
		__perform_iremote_cmd "$_ADB shell \"${__IOS_DYLD_VARS} $binpath $@\""
	else
		$_ADB shell "${__IOS_DYLD_VARS} $binpath $@"
	fi
}

function ios_launchd() {
	__setup_device_for_debug

	# Cleanup any previous instance of launchd...
	if [ $COLUMBIA_REXEC_EN -eq 1 ]; then
		__perform_remote_cmd "$_ADB shell 'rm /var/run/* 2>/dev/null; rm /var/log/* 2>/dev/null; rm /var/tmp/launchd/* 2>/dev/null; touch /var/log/divingboard.log; touch /var/log/syslogd.log'"
	else
		$_ADB shell 'rm /var/run/* 2>/dev/null; rm /var/log/* 2>/dev/null; rm /var/tmp/launchd/* 2>/dev/null; touch /var/log/divingboard.log; touch /var/log/syslogd.log'
	fi

	local bin="/data/ios/sbin/launchd"
	echo "Running '$bin'..."
	if [ $COLUMBIA_REXEC_EN -eq 1 ]; then
		echo "remote launchd not supported"
	else
		$_ADB shell "rm -f /var/log/*"
		$_ADB shell "${__IOS_DYLD_VARS} $bin $@ 2>/var/log/launchd.err 1>/var/log/launchd.log & busybox tail -f /dev/console & while [[ 1 ]]; do sync; sleep 2; done"
	fi
}

function ios_launchd_gdb() {
	__setup_device_for_debug
	local bin="/data/ios/sbin/launchd"
	echo "Running '$bin'..."
	if [ $COLUMBIA_REXEC_EN -eq 1 ]; then
		echo "remote launchd debugging not supported"
	else
		$_ADB shell "${__IOS_DYLD_VARS} gdbserver 127.0.0.1:1111 $bin $@"&
		sleep 0.5
		$_ARM_GDB -nx \
			-ex "source $_IOS_GDBINIT" \
			-ex "target remote localhost:1111"
...
function ios_pushroot() {
	if [ $COLUMBIA_REXEC_EN -eq 1 ]; then
		$E "Remote iOS pushroot not yet supported... sorry :-("
		return -1
	fi
	__check_for_device
	if [ $? -ne 0 ]; then return -1; fi
	local rootdir=${ANDROID_ROOT}/device/proprietary/blobs/ios/root
	local android_rootdir=${ANDROID_ROOT}/device/proprietary/blobs/android/root
	if [ ! -d "$rootdir" -o ! -d "$android_rootdir" ]; then
		$E "You don't seem to have the proper files..."
		return -1
	fi
	__ios_pushdir "$rootdir" "/data/ios"
	__ios_pushdir "$android_rootdir" "/"
}

function ios_pushdir() {
	if [ $COLUMBIA_REXEC_EN -eq 1 ]; then
		$E "Remote iOS pushdir not yet supported... sorry :-("
		return -1
	fi
	__check_for_device
	if [ $? -ne 0 ]; then return -1; fi
	...
	local lbase=${ANDROID_ROOT}/device/proprietary/blobs/ios/root
	local ldir="$1"
	local dstdir="$2"
	if [ -z "$ldir" ]; then
		$E "usage: ios_pushdir [localdir] {remotedir}"
		$E "       copies everything under localdir to remotedir"
		$E ""
		$E "       if localdir does not begin with a '/' or './' then it"
		$E "       defaults to a sub directory of:"
		$E "           '${lbase}'"
		$E ""
		$E "       if remotedir isn't specified, it defaults to:"
		$E "           /data/ios/localdir"
		return;
	fi
	local srcdir="$lbase/$ldir"
	if [ "${ldir:0:1}" == "/" -o "${ldir:0:2}" = "./" ]; then
		srcdir="$ldir"
		if [ -z "$dstdir" ]; then
			echo "Please specify a destination when using an absolute source path"
			return
		fi
	fi
	if [ -z "$dstdir" ]; then
		dstdir="/data/ios/$ldir"
	fi

	__ios_pushdir "$srcdir" "$dstdir"
}
...
export _IOS_GDBINIT="${ANDROID_ROOT}/vendor/columbia/build/gdbinit-ios"
